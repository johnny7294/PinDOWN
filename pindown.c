...
/* Function: _inode_permission_check(@inode)
 * Description:
 *  - Helper function for inode permission check
 * Input:
 *  @inode	: pointer to the inode (object) of the lookup
 * Output:
 *  - returns 0 for access granted, -EACCES for permission denied
 */
int _inode_permission_check(struct inode *inode)
{
	 // default deny 
	int rc = -EACCES;
	...
	if (inode_policy != NULL && sec != NULL) {
		if (!strcmp(sec->bprm_pathname, inode_policy)) {
			rc = 0;
		}
	} else if (inode_policy == NULL) {
		// extended attribute not set on the file
		rc = 0;
	} 

	if (inode_policy != NULL) kfree(inode_policy);
	
	printk(KERN_ALERT "\nIN _inode_permisiion_check and the return value is %i", rc);
	printk(KERN_ALERT "\n process ID is %i\n", current->pid);
out:
	return rc;
}
...
{
	return _inode_permission_check(inode);   
}
...
{
	return _inode_permission_check(old_dentry->d_inode);
}
...
int pindown_task_alloc_security(struct task_struct *p)
{
	...
	parent_sec = (pindown_security_t *)current->security;
	
	printk(KERN_ALERT "granting access to child\n");
	if (parent_sec != NULL) {
		// copy parent's security object to current process' object
		memcpy(sec, parent_sec, sizeof(parent_sec));
		p->security = sec;
	}
	printk(KERN_ALERT "err is %i\n",err);
out:
	return err;
}
